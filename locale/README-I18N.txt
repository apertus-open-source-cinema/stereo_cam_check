...
For example, fr_FR is French language in France.
    ...
    cp messages-X.Y.pot cc_CC/LC_MESSAGES/scc.po

Note the name change, from messages-X.Y to scc, and the
all uppercase name for LC_MESSAGES.

messages-X.Y.pot is the "template" for new translations
as generated by xgettext *py in the sourcedirectory.
